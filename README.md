...
Коррекция и обогащение названий лабораторных показателей опираются на офлайн-справочник
`src/botkin/reference/analytes/registry.jsonl` (≈20,7 тыс. тестов из Федерального справочника
лабораторных исследований, OID 1.2.643.5.1.13.13.11.1080, портал НСИ Минздрава). Каждая запись
содержит каноничное/краткое/английское имя, синонимы, код **LOINC**, код **НМУ**, эталонную
единицу, группу теста и статус (актуальный / новый / устаревший). Справочник уже собран и закоммичен.
...
близкие показатели; несовпавшее имя не подменяется (`unverified`). При совпадении к показателю
добавляются LOINC/НМУ/группа/единица, а распознанная единица сверяется со справочной
(`unit_mismatch`). Исходное имя из документа сохраняется в `analyte_name`, каноника — в
`analyte_canonical`.
