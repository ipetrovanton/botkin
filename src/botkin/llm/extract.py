    ...
    try:
        if path.suffix.lower() == ".pdf":
            ...
            for i, page in enumerate(doc):
                if i >= MAX_PAGES:
                    log.warning("PDF has more than %d pages, processing only first %d", MAX_PAGES, MAX_PAGES)
                    break
    ...
    try:
        ...
        if source_path and source_path.exists():
            try:
                ...
                source_path.with_suffix(".txt").write_text(raw_text, encoding="utf-8")
            except Exception as e:
...
def run_prescription(source_path: Path) -> list[Prescription]:
    b64_images = _pdf_to_base64_images(source_path)
    content: list[dict] = [{"type": "text", "text": "Extract prescriptions from these document images."}]
    for b64 in b64_images:
        content.append({"type": "image_url", "image_url": {"url": f"data:image/jpeg;base64,{b64}"}})
    messages = [
...
def run_doctor_report(source_path: Path) -> list[DoctorReport]:
    b64_images = _pdf_to_base64_images(source_path)
    content: list[dict] = [{"type": "text", "text": "Extract doctor reports from these document images."}]
    for b64 in b64_images:
        content.append({"type": "image_url", "image_url": {"url": f"data:image/jpeg;base64,{b64}"}})
    messages = [
