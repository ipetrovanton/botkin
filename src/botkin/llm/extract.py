...
from botkin.llm.prompts import (
    ...
    ANALYSIS_VLM_SYSTEM,
    DOCTOR_REPORT_INSTRUCTION,
    DOCTOR_REPORT_VLM_SYSTEM,
    PROMPTS_VERSION,
)
...
from botkin.parsing.text_layer import (
    _parse_text_line,
    _verbatim_guard,
    completeness_guard,
)
...
def _prepare_b64(source_path: Path) -> list[str]:
    ...
    if not b64_images:
        log.warning(
            "[EXTRACT_INPUT] Doc: '%s' | НЕТ изображений после препроцессинга — VLM нечего анализировать",
            source_path.name,
        )
    return b64_images
...
def _build_messages(
    system_prompt: str, instruction: str, source_path: Path
) -> list[dict]:
...
def _call_vlm(
    ...
    log.info(
        "[START_EXTRACT] Doc: '%s' | Type: '%s' | Model: %s",
        doc_name,
        doc_type,
        VLM_MODEL,
    )
    ...
    try:
        with timed("EXTRACT", doc_name) as t:
            ...
            t["metrics"] = metrics_of(
                response, model_name(VLM_MODEL), elapsed_call, num_ctx=VLM_NUM_CTX
            )
        ...
        if n_parsed == 0:
            log.warning(
                ...
                doc_name,
                len(raw),
                raw[:RAW_LOG_LIMIT] or "<пусто/недоступно>",
            )
        else:
            log.debug(
                "[RAW_EXTRACT] Doc: '%s' | сырой ответ (%d симв.): %s",
                doc_name,
                len(raw),
                raw[:RAW_LOG_LIMIT],
            )
        ...
        err.raw_text = _raw_text_from_exc(
            e
        )  # сырой ответ для возможного salvage обрезанного JSON
        raise err from e
...
def _vlm_extract_attempt(
    ...
    try:
        raw = _call_vlm(
            messages, RawAnalysis, doc_name, "analysis", structured=structured
        )
        ...
        if rows:
            log.info(
                "[EXTRACT_SALVAGED] Doc: '%s' | из обрезанного ответа спасено строк: %d",
                doc_name,
                len(rows),
            )
            return rows, 1
    ...
    log.info(
        "[EXTRACT_FALLBACK] Doc: '%s' | harvester собрал строк: %d (таблиц: %d)",
        doc_name,
        len(rows),
        len(tables),
    )
...
def _ocr_then_structure(
    ...
    if is_androflor_text(text):
        ...
        log.info(
            "[ANDROFLOR_OCR_SKIP] Doc: '%s' | не таблица Андрофлор (строк=%d) — пропуск",
            doc_name,
            len(rows),
        )
        return [], 0
    if is_sibr_text(text):
        ...
        if len(rows) >= _SIBR_MIN_ROWS:
            ...
            return rows, len(rows)
        log.info(
            "[SIBR_OCR_SKIP] Doc: '%s' | не таблица СИБР (строк=%d) — пропуск",
            doc_name,
            len(rows),
        )
        return [], 0
    ...
    if recovered:
        log.info(
            ...
            doc_name,
            len(recovered),
            [r.analyte_name for r in recovered],
        )
...
def _vlm_extract_with_retry(
    ...
    """
    messages = _messages_from_images(
        ANALYSIS_VLM_SYSTEM, ANALYSIS_INSTRUCTION, b64_images
    )
    rows, tables = _vlm_extract_attempt(messages, doc_name)
    if not rows and VLM_STRUCTURED_OUTPUT:
        log.info(
            "[EXTRACT_UNSTRUCTURED_RETRY] Doc: '%s' | structured VLM пуст — повтор без grammar",
            doc_name,
        )
...
def _supplement_with_vlm(
    ocr_rows: list[LabResult], b64_images: list[str], doc_name: str
) -> list[LabResult]:
    ...
    """
    log.info(
        "[EXTRACT_VLM_SUPPLEMENT] Doc: '%s' | text-LLM 0 строк — structured VLM как доп. источник",
        doc_name,
    )
    ...
    log.info(
        "[EXTRACT_VLM_SUPPLEMENTED] Doc: '%s' | OCR=%d + VLM=%d → объединено=%d",
        doc_name,
        len(ocr_rows),
        len(vlm_rows),
        len(merged),
...
def _extract_once(
    ...
    structured_count = 0
    try:
        rows, structured_count = _ocr_then_structure(
            b64_images, doc_name, low_res_retry_fn
        )
    except Exception as e:  # noqa: BLE001 — OCR-вызов вне instructor-ретраев; падение → фолбэк на VLM
        log.warning(
            "[OCR_PRIMARY_FAILED] Doc: '%s' | OCR-путь упал, фолбэк на VLM: %s",
            doc_name,
            e,
        )
        rows = []
    if rows:
        ...
        return rows, 1
    log.info(
        "[EXTRACT_VLM_FALLBACK] Doc: '%s' | OCR-путь пуст — structured VLM", doc_name
    )
    return _vlm_extract_with_retry(b64_images, doc_name)


def _finish(
    rows: list[LabResult], doc_name: str, t0: float, n_calls: int
) -> list[LabResult]:
...
def run_analysis(source_path: Path) -> list[LabResult]:
    ...
    def _low_res_retry_fn(page_index: int | None) -> Callable[[], list[str]]:
        def _render() -> list[str]:
            images = prepare_images(
                source_path,
                long_side=_ANDROFLOR_RETRY_LONG_SIDE,
                upscale=False,
                deskew=False,
                enhance=False,
            )
    ...
    if n_pages <= 1:
        ...
        try:
            rows, _ = _extract_once(
                b64_images, source_path.name, _low_res_retry_fn(None)
            )
        except ExtractionError as e:
            log.warning(
                "[EXTRACT_FAILED] Doc: '%s' | извлечение пусто: %s", source_path.name, e
            )
            rows = []
        n_calls = 1
    else:
        ...
        log.info(
            "[MULTIPAGE] Doc: '%s' | страниц=%d — извлечение постранично",
            source_path.name,
            n_pages,
        )
        rows = []
        ...
        for i, page in enumerate(b64_images):
            n_calls += 1
            try:
                page_rows, _ = _extract_once(
                    [page], f"{source_path.name}#стр{i + 1}", _low_res_retry_fn(i)
                )
            except ExtractionError as e:
                log.warning(
...
def run_doctor_report(source_path: Path) -> list[DoctorReport]:
    ...
    messages = _messages_from_images(
        DOCTOR_REPORT_VLM_SYSTEM,
        DOCTOR_REPORT_INSTRUCTION,
        to_base64_jpegs(images),
    )
    reports = _call_vlm(
        messages, DoctorReports, source_path.name, "doctor_report"
    ).results
